...
<div x-data="categoryManagement()" x-init="init()" x-cloak class="space-y-8 relative">
    ...
    <button @click="showSidebar = true"
        ...
        <span style="writing-mode: vertical-rl;" class="text-[9px] font-black uppercase tracking-[0.3em] rotate-180 drop-shadow-md text-indigo-50">Category Filters</span>
    </button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 gap-y-10 mt-4">
        ...
        <div class="relative flex flex-col bg-gradient-to-br from-indigo-50 to-blue-50 rounded-2xl shadow-lg shadow-indigo-500/20 border border-indigo-200 hover:-translate-y-2 transition-all duration-300 group cursor-pointer"
            ...
            <div class="p-4 text-right pt-4">
                <p class="text-xs font-bold tracking-wider text-blue-500 uppercase">Total Categories</p>
            ...
            <div class="mx-4 mb-4 border-t border-indigo-200 pt-2">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-[10px] text-indigo-700 font-bold uppercase tracking-tight">System-wide Categories</span>
                </div>
            </div>
        </div>
        ...
        <div class="relative flex flex-col bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl shadow-lg shadow-emerald-500/20 border border-emerald-200 hover:-translate-y-2 transition-all duration-300 group cursor-pointer"
            ...
            <div class="p-4 text-right pt-4">
                <p class="text-xs font-bold tracking-wider text-emerald-500 uppercase">Active</p>
                <h4 class="text-3xl font-bold text-emerald-700 drop-shadow-sm font-mono" x-text="stats.active">0</h4>
            </div>
            <div class="mx-4 mb-4 border-t border-emerald-200 pt-2 text-emerald-700">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-[10px] font-bold uppercase tracking-tight">Available For Use</span>
                </div>
            </div>
        </div>

        <!-- Inactive Categories Card -->
        <div class="relative flex flex-col bg-gradient-to-br from-rose-50 to-red-50 rounded-2xl shadow-lg shadow-rose-500/20 border border-rose-200 hover:-translate-y-2 transition-all duration-300 group cursor-pointer"
             @click="filterStatus = 'inactive'; searchCategories()">
            <div class="absolute -top-6 left-4 h-14 w-14 grid place-items-center rounded-xl bg-gradient-to-tr from-rose-600 to-red-400 shadow-lg shadow-rose-900/30 border border-rose-300 group-hover:scale-110 transition-transform duration-300">
            ...
            <div class="mx-4 mb-4 border-t border-rose-200 pt-2 text-rose-700">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-[10px] font-bold uppercase tracking-tight">Currently Disabled</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-6 transition-all duration-300" :class="showSidebar ? 'lg:col-span-9' : 'lg:col-span-12'">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 text-sm font-medium mt-1">Classify and organize your pharmacy inventory</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-4 items-center">
                        ...
                        <div class="flex items-center gap-2 bg-white border border-indigo-100 rounded-xl px-3 py-1.5 shadow-sm">
                            ...
                            <select x-model="pagination.per_page" @change="searchCategories()" class="bg-transparent text-indigo-600 text-[10px] font-black uppercase cursor-pointer outline-none focus:ring-0 border-none p-0 pr-4">
            ...
            <div x-show="selectedIds.length > 0" 
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="bulkUpdateStatus(true)" class="px-3 py-1.5 bg-emerald-500/80 hover:bg-emerald-500 text-white rounded-lg text-[9px] font-black uppercase tracking-widest transition-all">Activate</button>
                        <button @click="bulkUpdateStatus(false)" class="px-3 py-1.5 bg-white/20 hover:bg-white/30 text-white rounded-lg text-[9px] font-black uppercase tracking-widest transition-all">Deactivate</button>
                            ...
                            <tr>
                                <th class="px-5 py-5 w-10">
                                    <div class="flex items-center justify-center">
                                        <input type="checkbox" @change="toggleAll($event)" :checked="selectedIds.length === categories.length && categories.length > 0"
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <template x-if="!loading && categories.length === 0">
                                <tr>
                                    <td colspan="6" class="py-24 text-center">
                                        ...
                                        <h3 class="text-2xl font-black text-slate-800 mb-2">No Categories Found</h3>
                            ...
                            <template x-for="category in categories" :key="category.id">
                                <tr class="hover:bg-blue-50/40 transition-colors group">
                                    ...
                                    <td class="px-5 py-4">
                                        <p class="text-xs font-bold text-slate-600 line-clamp-2 max-w-xs" x-text="category.description || '-'"></p>
                                    </td>
                                    <td class="px-5 py-4 text-center whitespace-nowrap">
                                        <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 text-slate-700 font-bold text-sm border border-slate-200" x-text="category.display_order"></span>
                                    </td>
                                    <td class="px-5 py-4 text-center">
                                        <button @click="toggleStatus(category)" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" :class="category.is_active ? 'bg-indigo-500' : 'bg-slate-200'">
                                            <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ease-in-out shadow-sm" :class="category.is_active ? 'translate-x-6' : 'translate-x-1'"></span>
                                        </button>
                                        <div class="text-[10px] font-bold uppercase tracking-wider mt-1" :class="category.is_active ? 'text-indigo-600' : 'text-slate-400'" x-text="category.is_active ? 'Active' : 'Inactive'"></div>
                                    </td>
                                    <td class="px-5 py-4 text-center whitespace-nowrap">
            ...
            {{-- Premium Pagination --}}
            <div x-show="!loading && categories.length > 0" class="p-8 bg-slate-50 border-t border-slate-100 mt-auto rounded-b-3xl">
                ...
                <div class="overflow-y-auto scrollbar-hide flex-1 space-y-5 p-5" style="scrollbar-width: none;">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            <template x-if="searchQuery">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-lg text-[9px] font-black uppercase tracking-widest group">
                                    ...
                                    <button @click="searchQuery=''; searchCategories()" class="hover:text-rose-600 transition-colors"><i class="fas fa-times-circle"></i></button>
                                </span>
                            </template>
                            <template x-if="filterStatus">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-lg text-[9px] font-black uppercase tracking-widest group">
                                    ...
                                    <button @click="filterStatus=''; searchCategories()" class="hover:text-rose-600 transition-colors"><i class="fas fa-times-circle"></i></button>
                                </span>
                            </template>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="relative group">
                            ...
                            <i class="fas fa-tag absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500 transition-colors"></i>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <select x-model="filterStatus" @change="searchCategories()"
            ...
            <div x-show="showAddModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" class="relative inline-block w-full max-w-lg overflow-hidden text-left align-bottom transition-all transform bg-white shadow-2xl rounded-3xl sm:my-8 sm:align-middle border border-slate-100">
                ...
                <div class="px-6 py-5 border-b border-indigo-100/50 bg-gradient-to-r from-blue-50 to-indigo-50 flex justify-between items-center relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-black text-slate-800 tracking-tight relative z-10" x-text="editing ? 'Edit Category' : 'Create Category'"></h3>
                    </div>
                ...
                <div class="px-6 py-5 max-h-[70vh] overflow-y-auto">
                    <form @submit.prevent="saveCategory" class="space-y-5">
                        <div class="space-y-1.5 focus-within:text-indigo-600 transition-colors">
                            <label class="text-[10px] uppercase tracking-widest font-black text-slate-500 ml-1 flex items-center gap-2">Category Name <span class="text-rose-500">*</span></label>
                        ...
                        <div class="space-y-1.5 focus-within:text-indigo-600 transition-colors">
                            <label class="text-[10px] uppercase tracking-widest font-black text-slate-500 ml-1 flex items-center gap-2">Description</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-1.5 focus-within:text-indigo-600 transition-colors">
                                <label class="text-[10px] uppercase tracking-widest font-black text-slate-500 ml-1 flex items-center gap-2">Display Order</label>
                                <input type="number" x-model="form.display_order" placeholder="0"
                                    class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-400/10 transition-all font-bold text-slate-800 placeholder:text-slate-400">
                            </div>

                            <div class="space-y-1.5 flex flex-col justify-center">
                                <label class="text-[10px] uppercase tracking-widest font-black text-slate-500 ml-1 mb-2">Category Status</label>
                                <label class="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <span class="ml-3 text-sm font-bold text-slate-700" x-text="form.is_active ? 'Active' : 'Inactive'"></span>
                                </label>
                            </div>
                        </div>
                    </form>
    ...
    <div x-show="showDeleteModal" class="fixed inset-0 z-[70] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="showDeleteModal" class="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-md sm:w-full sm:p-6 text-center border border-slate-100">
                ...
                <h3 class="text-xl font-black text-slate-800 mb-2">Delete Category</h3>
            ...
            <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg z-10 overflow-hidden relative border border-slate-100" x-transition.scale>
                    ...
                    <div class="flex items-center justify-between relative z-10">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-slate-300 text-[10px] font-black uppercase tracking-widest mt-1">Category Data Record</p>
                            </div>
                        </div>
                ...
                <div class="p-8 space-y-6">
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Slug (URL string)</p>
                            <p class="font-mono text-sm text-slate-800 font-bold mt-1" x-text="dataToView?.slug || '-'"></p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Display Order</p>
                            <div class="flex items-center mt-1">
                                <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-black bg-indigo-50 text-indigo-600 border border-indigo-100" x-text="dataToView?.display_order || '0'"></span>
                            </div>
                        </div>
                        <div class="space-y-1 col-span-2 relative">
                            ...
                            <p class="text-sm font-medium text-slate-700 mt-1 pr-4" x-text="dataToView?.description || 'No description provided'"></p>
                        </div>
                        <div class="space-y-1">
                            ...
                            <div class="flex items-center gap-2 mt-1">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider"
                                    ...
                                    <i class="fas" :class="dataToView?.is_active ? 'fa-check-circle' : 'fa-power-off'"></i>
    ...
    function categoryManagement() {
        return {
            ...
            closeAddModal() {
                ...
                setTimeout(() => { this.form = { id: null, name: '', description: '', display_order: 0, is_active: true }; }, 300);
            },
            ...
            async saveCategory() {
                ...
                this.saving = true;
                const url = this.editing ? `/pharmacy/medicine-categories/${this.form.id}` : '/pharmacy/medicine-categories';
            ...
            async bulkUpdateStatus(isActive) {
                if (!confirm(`Are you sure you want to ${isActive ? 'activate' : 'deactivate'} ${this.selectedIds.length} categories?`)) return;
            ...
            async bulkDelete() {
                if (!confirm(`Are you sure you want to permanently purge ${this.selectedIds.length} categories?`)) return;
